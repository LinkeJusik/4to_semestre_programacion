                    ...
                    int iCmdShow)
{
    ...
    if (strlen(lpCmdLine)==0) miParametro="Dragon";
	...
	if(strncmp(miParametro,"Sierpinski",7)==0){  
		iniciaSierpinski(5.5, 0.19, -6.42, 5.4,&miTortuga);
		miFractal = dibujaSierpinski;
	}
	if(strncmp(miParametro,"Hilbert",7)==0){  
		iniciaHilbert(5.0, 0.26, -5.0, -3.0, &miTortuga);
		miFractal = dibujaHilbert;
	}
	if(strncmp(miParametro,"Dragon",6)==0){  
		iniciaDragon(16.0, 0.03, -3.0, 2.0, &miTortuga);
	...
	if(strncmp(miParametro,"NestedTriangule",15)==0){  
		iniciaNest(8,15,-7.9,5.8,&miTortuga);
