...
{
  tortuga->grados = (((tortuga->grados-g)%360)+360)%360; //rango de 0 o menor de 360
...
int avanza(double l, LOGO* tortuga)
{
  ...
  tortuga->x += l*cos(tortuga->grados*G2R);  //proyecciones
  tortuga->y += l*sin(tortuga->grados*G2R);
  if (tortuga->pluma == ABAJO)
    tortuga->dibujaSegmento(x, y, tortuga->x, tortuga->y); //une las proyecciones con lineas
