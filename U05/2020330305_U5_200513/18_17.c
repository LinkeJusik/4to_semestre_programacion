...
int insertaI(INFO info, ARBOL *pa){
  ...
  aux = *pa;
	...
	do {
		if((pnodo->info.ent_1 > aux->info.ent_1)||((pnodo->info.ent_1 == aux->info.ent_1)&&(pnodo->info.ent_2 < aux->info.ent_2))){ //Primer entero descendente y segundo entero ascendente
			if(aux->izq == NULL){
				aux->izq = pnodo;
				break;
			}
			aux = aux->izq;
    }else if((pnodo->info.ent_1 == aux->info.ent_1)&&(pnodo->info.ent_2 == aux->info.ent_2)){ //conctena cadenas y suma float
...
int buscaI(INFO info, ARBOL a){
	...
	while(a!=NULL){
      ...
      return 1;
		...
		if((info.ent_1 > a->info.ent_1)||((info.ent_1 == a->info.ent_1)&&(info.ent_2 < a->info.ent_2))
      ||((info.ent_1 == a->info.ent_1)&&(info.ent_2 == a->info.ent_2)))
      a = a->izq;
		else a = a->der;
	}
	return 0;
...
INFO LectI(char leyenda[],INFO info){
  ...
  while(rspt==2){
    printf("\n\tESCRIBA LA INFORMACION A %s\n\n"
      "\tEscriba una cadena de caracteres:  ", leyenda); gets(aux); gets(info.cad_1);
    printf("\tEscriba otra cadena de caracteres: "); gets(info.cad_2);
    printf("\n\tEscriba un numero entero:  "); scanf("%d",&info.ent_1);
    printf("\tEscriba otro numero entero: "); scanf("%d",&info.ent_2);
    printf("\n\tEscriba un numero real: "); scanf("%lg",&info.real);
