...
typedef struct _elemento_lista_{
    ...
    struct _elemento_lista_ *ini;
    struct _elemento_lista_ *fin;
...
int recorrer(LISTA l){
    ...
    while(b==0){
    printf("Actualmente estás en:\n[%3d, %3d, %3lg, %s, %s]", l->info.entero1, l->info.entero2, l->info.real, l->info.cadena1, l->info.cadena2);
    ...
    switch(a){
        ...
        if(l==NULL){
            laux2=laux3;
            for(j=0; laux2!=NULL, j<1000; j++, laux2=laux2->sig){
                if(laux2->sig==NULL){
                    l=laux2;
                    j=100001;
                }
